...
export function DashboardPage({ onSelectLevel, isSoundOn, onToggleSound, onPlayClick, unlockedLevel = 1 }: DashboardPageProps) {
  ...
  const handleCardClick = (levelId: number) => {
    if (levelId > unlockedLevel) {
      onPlayClick();
      setLockToast(`🔒 Selesaikan Level ${levelId - 1} terlebih dahulu untuk membuka level ini!`);
  ...
  return (
    <div id="dashboard-page" className="h-screen w-full flex flex-col justify-center items-center relative overflow-hidden antialiased py-1.5 px-2 sm:py-3 sm:px-4 lg:py-6 lg:px-8 xl:py-8 select-none">
            ...
            {levels.map((level) => {
              ...
              return (
                <div
                  ...
                  className={`${
                    isLocked
                      ? 'bg-slate-100/70 backdrop-blur-md border-[3px] border-slate-300/80 opacity-80 cursor-not-allowed hover:border-slate-400'
                      : `${theme.cardBg} shadow-sm hover:shadow-xl ${theme.hoverBorder} ${theme.hoverGlow} hover:-translate-y-1 active:scale-98 cursor-pointer`
                  } rounded-lg sm:rounded-2xl md:rounded-3xl 2xl:rounded-4xl p-2 sm:p-3 md:p-5 lg:p-6 2xl:p-8 text-left flex flex-col justify-between transition-all duration-300 group relative overflow-hidden w-full min-h-0 sm:min-h-[160px] md:min-h-[220px] lg:min-h-[250px] 2xl:min-h-[300px]`}
                >

                  {/* Card Top: Level number + difficulty / Locked Badge */}
                  <div className="flex items-start justify-between shrink-0">
                    <div className="flex flex-col">
                      ...
                      <span className={`text-base sm:text-2xl md:text-3xl lg:text-4xl 2xl:text-5xl font-black font-display leading-none ${isLocked ? 'text-slate-400' : theme.accentText}`}>
                        {level.id < 10 ? `0${level.id}` : level.id}
                      </span>
                    </div>
                    {isLocked ? (
                      <span className="text-[6.5px] sm:text-[8px] md:text-xs lg:text-sm 2xl:text-base font-black px-1.5 sm:px-2.5 md:px-3 py-0.5 lg:py-1 2xl:py-1.5 2xl:px-4 rounded-full bg-slate-500/80 text-white leading-none flex items-center gap-1 shadow-xs font-display">
                        <Lock className="w-2 sm:w-3 md:w-3.5 h-2 sm:h-3 md:h-3.5" />
                        Terkunci
                      </span>
                    ) : (
                  ...
                  <div className="my-1 sm:my-2 md:my-3 lg:my-5 2xl:my-6 shrink-0">
                     <h3 id={`dashboard-level-title-${level.id}`} className={`font-black ${isLocked ? 'text-slate-500' : titleColorClass(level.id)} text-[9px] sm:text-xs md:text-base lg:text-lg 2xl:text-2xl leading-tight font-display tracking-wide transition-colors duration-300 pr-1 sm:pr-3 md:pr-8 truncate sm:whitespace-normal`}>
                  ...
                  <div className="mt-auto shrink-0 flex flex-col">
                    {isLocked ? (
                      <button
                        id={`dashboard-level-play-btn-${level.id}`}
                        type="button"
                        className="w-full py-1 sm:py-1.5 md:py-2.5 2xl:py-3.5 pl-1.5 sm:pl-3 md:pl-5 2xl:pl-6 pr-1 sm:pr-1.5 md:pr-2.5 2xl:pr-4 rounded-md sm:rounded-xl md:rounded-2xl 2xl:rounded-3xl text-slate-500 font-bold text-[7.5px] sm:text-[9px] md:text-xs 2xl:text-base flex items-center justify-between shadow-xs bg-slate-200/90 border border-slate-300 cursor-not-allowed"
                      >
                        <span className="font-display tracking-wider flex items-center gap-1">
                          <span className="hidden sm:inline">Buka di </span>Level {level.id - 1}
                        </span>
                        <span className="w-3.5 h-3.5 sm:w-5 sm:h-5 md:w-6 md:h-6 2xl:w-9 2xl:h-9 bg-slate-300/80 rounded-full flex items-center justify-center shrink-0">
                          <Lock className="w-2 sm:w-3 h-2 sm:h-3 text-slate-500" />
                        </span>
                      </button>
                    ) : (
                      <button
                        id={`dashboard-level-play-btn-${level.id}`}
                        type="button"
                        className={`w-full py-1 sm:py-1.5 md:py-2.5 2xl:py-3.5 pl-1.5 sm:pl-3 md:pl-5 2xl:pl-6 pr-1 sm:pr-1.5 md:pr-2.5 2xl:pr-4 rounded-md sm:rounded-xl md:rounded-2xl 2xl:rounded-3xl text-white font-bold text-[7.5px] sm:text-[9px] md:text-xs 2xl:text-base flex items-center justify-between shadow-xs sm:shadow-md transition-all duration-300 hover:shadow-lg active:scale-98 cursor-pointer group/btn border border-white/10 ${theme.btnBg}`}
                      >
      ...
      {lockToast && (
        <div
          id="dashboard-lock-toast"
          className="absolute bottom-4 sm:bottom-6 left-1/2 -translate-x-1/2 z-50 bg-[#0090D4] backdrop-blur-md border-[2.5px] sm:border-[3px] md:border-[4px] 2xl:border-[6px] border-white rounded-lg sm:rounded-2xl shadow-lg sm:shadow-2xl pt-1.5 pb-1 px-3.5 sm:pt-2.5 sm:pb-2 sm:px-6 md:pt-3 md:pb-2.5 md:px-8 2xl:pt-4 2xl:pb-3 flex items-center justify-center text-[10px] sm:text-xs md:text-sm 2xl:text-base font-black font-display tracking-wide text-white drop-shadow-md animate-fade-in select-none whitespace-nowrap"
        >
          <span>{lockToast}</span>
        </div>
      )}
